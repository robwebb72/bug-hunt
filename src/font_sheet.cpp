...
// TODO: [RW:2020-06-29]  This code is rather fragile and might fall down if the file is in an unexpected format.  Add error handling!
...
namespace font {
	...
	void Font::LoadFontMetrics(std::string filename) {
	...
	void Font::LoadResources(std::string font_name) {
	...
	void Font::Print(olc::PixelGameEngine* engine, olc::vi2d pos, std::string text, const olc::Pixel& tint) {
		if (text.size() == 0) return;
		for (char& c : text) {
			if (c == '\n') {
				// TODO: [RW:2020-07-03] Need to be able to move to new line here - I need the font line height for this
				pos.y += 10;
				continue;
			}
			try {
				FontMetricChar* metric = font_map.at(c);
